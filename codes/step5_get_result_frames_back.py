...
"""
@author: LiGen
@file: step5_get_result_frames_back.py
@time: 2021/9/3 14:00
@describe: 
"""
import argparse
...
def prepare_input_video(video_path, frame_root, rect_root, mask_root, back_root, fore_root):
    print("# step5: extract some frames, background, masks, foreground and rect.")
    print("# step5: from {}".format(video_path))
    ...
    video_cap.release()
    print("# step5: extract {} frames over".format(frame_num))

    frame_name_list = os.listdir(frame_root)
    frame_name_list.sort(reverse=False)
    print("# step5: frame num: {}".format(len(frame_name_list)))
    ...
    print("# step5: extract background, masks and foreground over.")
...
if __name__ == '__main__':
    ...
    arg('--source_video_path', type=str, default="", help="")
    arg('--source_frame_root', type=str, default="", help="")
    arg('--source_rect_root', type=str, default="", help="")
    arg('--source_mask_root', type=str, default="", help="")
    arg('--source_back_root', type=str, default="", help="")
    arg('--source_fore_root', type=str, default="", help="")
    args = parser.parse_args()

    prepare_input_video(video_path=args.source_video_path,
                        frame_root=args.source_frame_root,
                        rect_root=args.source_rect_root,
                        mask_root=args.source_mask_root,
                        back_root=args.source_back_root,
                        fore_root=args.source_fore_root)
